public class P11{
  ...
  {01, 70, 54, 71, 83, 51, 54, 69, 16, 92, 33, 48, 61, 43, 52, 01, 89, 19, 67, 48}
    ...
    for (int i = 0; i < noRows; i++){
      for (int j = 0; j < noCols - 3; j++){
        int curr = mat[i][j] * mat[i][j+1] * mat[i][j + 2] * mat[i][j + 3];
        if (curr > largest) largest = curr;
      }
    }
    //then columns
    for (int i = 0; i < noCols; i++){
      for (int j = 0; j < noRows - 3; j++){
        int curr = mat[j][i] * mat[j+1][i] * mat[j+2][i] * mat[j+3][i];
        if (curr > largest) largest = curr;
      }
    }
